...
impl Repository {
    ...
    async fn upload_local(&self, package_name: &str, content: Bytes) -> Result<()> {
        info!("Uploading {} to local repository at {}", package_name, self.path);
    ...
    async fn upload_s3(&self, package_name: &str, content: Bytes) -> Result<()> {
        info!("Uploading {} to S3 repository at {}", package_name, self.path);
        
        // Parse bucket and key from path
        let parts: Vec<&str> = self.path.trim_start_matches("s3://").splitn(2, '/').collect();
        let bucket = parts.get(0).ok_or_else(|| anyhow!("Invalid S3 path"))?;
